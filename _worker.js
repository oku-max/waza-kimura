...
function _aiBranchPrompt(ctx) {
  return `あなたはブラジリアン柔術(BJJ)に精通したアシスタントです。
この動画を視聴し、「相手がこう来たら、自分はこれができる」という対応関係を抽出してJSONで出力してください。
${ctx ? `\n【動画情報】\n${ctx}\n` : ''}
【最重要・「分岐」の定義】ここで言う分岐とは、**次の展開が生まれる発生ポイント＝相手のリアクションや姿勢の違い**のことです。相手がどう動く/構えるかによって、こちらが選べる技が変わる、その切り替わりの点を指します。
例: クローズドガードで「相手が前にプレッシャーをかけてくる」と「相手がアップライトで胸を張る」は別の分岐（かけられる技が違う）。
【禁止・混同しないこと】自分の手順の1ステップ（グリップを作る→フックを入れる→崩す…）を分岐にしないこと。分岐はあくまで「相手側の状態・反応」であって、自分の動作の途中経過ではない。1つのシチュエーションにつき分岐は数個（相手の反応の種類の数）に収めること。
【出力形式】次のJSONのみを出力（コードフェンス・説明文は禁止）:
{"items":[{"situation":"開始状況。ポジションと自分がトップ/ボトムか・主なグリップまで（例: デラヒーバ（ボトム）で襟袖グリップを確保）","trigger":"分岐＝相手の反応や姿勢（例: 相手が正面に戻ってくる / 相手が前にプレッシャーをかける / 相手が距離を取る）。相手が特に反応しない基本線なら「基本（相手が反応しない）」","response":"その時こちらができること・技（BJJで一般的な表記。例: 足をすくってスイープ）","detail":"実行のための手順の要点を1〜2文で簡潔に（ここに細かい手順を入れる。分岐にはしない）","timestamp":"その対応が映像で実演されている瞬間の M:SS"}]}
【ルール】
- trigger は必ず「相手側の動き・反応・姿勢」の視点で書くこと。自分の動作を trigger にしない。
- 同じ situation に対して trigger 違いの item を複数並べてよい（それが分岐の一覧になる）。
...
【記号ルール】大見出しは ◾️ を使うこと。小見出しや特に強調したい語句は **強調したい語** のように ** で囲んでよい（アプリ側で太字表示される）。ただし Markdownの見出し記号 # は使わないこと。箇条書きは行頭に「- 」を付けること。
