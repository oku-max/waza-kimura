...
<script>
  ...
  window.__notes = [{ id:'keep1', name:'既存ノートA', status:'wip', tags:['x'], blocks:[{type:'text',content:'消えたら困る'}] }];
...
await page.waitForTimeout(250);
check('ノートが1件増える', await page.evaluate(() => window.__notes.length) === 2);
check('作ったノートを開く', await page.evaluate(() => window.__opened) === 'n2');
...
check('増えただけ（3件）', await page.evaluate(() => window.__notes.length) === 3);
