...
function _cvUpdateSearch(view) {
  ...
  const match = window._matchQuery;
  if (rawQ.trim() && parse && match) {
    // organize.js と同じクエリ構文 (-除外 / "完全一致" / field:値 / フレーズ) を適用
    const parsed = parse(rawQ);
    filtered = videos.filter(v => match(v, parsed, null));
  } else {
...
const _CV_FIELD_TO_CK = { tb: 'tb', cat: 'cat', pos: 'pos', tags: 'tech' };

// names の値を条件に使っているリスト。names を null にすると、そのグループに条件があるもの全部。
window._cvListsUsingTags = function(names, fields) {
...
//    起こすため廃止。安全策が固まるまで何もしない（_views と localStorage は保持する）。
window._cvClearForLogout = function() { /* no-op: データ保全のため無効化 */ };
