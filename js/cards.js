...
export function cardHTML(v) {
  ...
  // カウンターバッジ (B案: 下段インライン)
  const _pc = v.practice || 0, _vc = v.views || 0;
  const _ago = (v.lastPracticed || v.lastViewed) ? (window.vpCntFormatAgo?.(v.lastPracticed || v.lastViewed) || '') : '';
  const cntBadges = `<div class="card-cnt" style="display:flex;gap:10px;padding:5px 10px 7px;font-size:10px;font-weight:700;align-items:center;line-height:1">
    ...
    <span style="display:inline-flex;align-items:center;gap:3px;color:${_vc>0?'#1971c2':'var(--text3)'};${_vc===0?'opacity:.55':''}">👁 ${_vc||'未'}</span>
