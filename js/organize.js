...
export function renderOrg() {
  ...
  function _buildRowHTML(v) {
    ...
    const scrollCells = visCols.map(col => {
      ...
      if (col === 'drill') { const _don=`<svg width="29" height="16" viewBox="0 0 35 20" fill="none"><rect x="1" y="1" width="33" height="18" rx="9" fill="#7c3aed"/><text x="17.5" y="14" text-anchor="middle" fill="white" font-size="9" font-weight="900" font-family="Arial Black,sans-serif" letter-spacing="0.8">ドリル</text></svg>`, _doff=`<svg width="29" height="16" viewBox="0 0 35 20" fill="none"><rect x="1" y="1" width="33" height="18" rx="9" fill="none" stroke="#555" stroke-width="1.5"/><text x="17.5" y="14" text-anchor="middle" fill="#555" font-size="9" font-weight="900" font-family="Arial Black,sans-serif" letter-spacing="0.8">ドリル</text></svg>`; return `<td class="org-td" data-col="drill" style="text-align:center;padding:4px"><button onclick="event.stopPropagation();orgTogDrill('${v.id}')" style="background:none;border:none;cursor:pointer;padding:2px;border-radius:4px;line-height:0;transition:opacity .15s;opacity:${v.drill?'1':'0.35'}" title="${v.drill?'Drill解除':'Drillに追加'}">${v.drill?_don:_doff}</button></td>`; }
      if (col === 'addedAt') {
...
export function orgTogDrill(id) {
  try {
    ...
    if (!v) return;
    v.drill = !v.drill;
    const _don  = `<svg width="29" height="16" viewBox="0 0 35 20" fill="none"><rect x="1" y="1" width="33" height="18" rx="9" fill="#7c3aed"/><text x="17.5" y="14" text-anchor="middle" fill="white" font-size="9" font-weight="900" font-family="Arial Black,sans-serif" letter-spacing="0.8">ドリル</text></svg>`;
    const _doff = `<svg width="29" height="16" viewBox="0 0 35 20" fill="none"><rect x="1" y="1" width="33" height="18" rx="9" fill="none" stroke="#555" stroke-width="1.5"/><text x="17.5" y="14" text-anchor="middle" fill="#555" font-size="9" font-weight="900" font-family="Arial Black,sans-serif" letter-spacing="0.8">ドリル</text></svg>`;
    const tr = document.getElementById('org-row-' + id);
    if (tr) {
...
const _INLINE_COLS = {
  ...
  status:    { field: 'status', type: 'radio', opts: () => ['未着手','理解','練習中','マスター'] },
};
